...
def nn_process_input_queue(nnQ, stereoQ, db, jpeg_root_folder, stereo_root_folder, capture_lock):
    stereoImageryMemoryControl = JpegMemoryControl(stereo_root_folder)
    while True:
        # grab frame from DB
        try: 
            ...
            if (len(images) > 0):
                ...
                if image_name not in currently_processing:
                    ...
                    if cv_frame is not None:
                        ...
                        # img.setTimestamp(ts) # preserving the timestamp of original
                        img.setWidth(640)
                        img.setHeight(640)
